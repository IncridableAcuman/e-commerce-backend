...
export const Admin: React.FC = () => {
  ...
  // Re-fetch va onSubmit uchun alohida funksiya
  ...
  // Komponent yuklanganda ishlaydigan asinxron effect
  useEffect(() => {
    let isMounted = true;

    productApi.getProducts(0, 50)
      .then((res) => {
        if (isMounted) {
          setProducts(res.data.content);
        }
  ...
  const onSubmit = async (data: CreateProductInput) => {
    try {
      ...
      await loadProducts(); // Mahsulotlar ro'yxatini yangilash
